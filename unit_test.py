...
def test_model_class():
    ...
    train_loader, val_loader = mgr.train.data.get_data(mgr.configration.load.load_configurations()['data_path'])
    assert len(train_loader.shape) == 4
    assert len(val_loader.shape) == 4
    assert train_loader.batch_size == mgr.configration.load.load_configurations()['train_batch_size']
    assert val_loader.batch_size == mgr.configration.load.load_configurations()['valid_batch_size']
    assert train_loader.shape[1:] == val_loader.shape[1:]
    assert train_loader.shape[1:] == (1, 128, 94)
    assert train_loader.shape[0] == mgr.configration.load.load_configurations()['train_size']
    assert val_loader.shape[0] == mgr.configration.load.load_configurations()['valid_size']
