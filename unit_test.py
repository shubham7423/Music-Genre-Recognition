...
def test():
    ...
    # test that the model outputs for 8 labels
    ...
    assert os.path.exists(mgr.configuration.load.load_configurations()['lstm']['train']['features_path'])
    assert os.path.exists(mgr.configuration.load.load_configurations()['lstm']['train']['labels_path'])
    assert os.path.exists(mgr.configuration.load.load_configurations()['transformer']['train']['features_path'])
    assert os.path.exists(mgr.configuration.load.load_configurations()['transformer']['train']['labels_path'])

    #if file exists, check if they are loaded properly, as the file is common, we can use any model's path
    features, labels = mgr.train.data.load_data(mgr.configuration.load.load_configurations()['cnn']['train']['features_path'], mgr.configuration.load.load_configurations()['cnn']['train']['labels_path']);
    ...
    assert os.path.exists(mgr.configuration.load.load_configurations()['preprocessing']['train'])
    assert os.path.exists(mgr.configuration.load.load_configurations()['preprocessing']['test'])
    ...
    assert os.path.exists(mgr.configuration.load.load_configurations()['cnn']['train']['save_model_at'])
    assert os.path.exists(mgr.configuration.load.load_configurations()['lstm']['train']['save_model_at'])
    assert os.path.exists(mgr.configuration.load.load_configurations()['transformer']['train']['save_model_at'])
