...
class MREData(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, data_csv, audio_dir, target_sample_rate=44100, num_samples=480000, device="cpu", mel_transformation=None, Amp2db=None):
    ...
    def __getitem__(self, idx):
        ...
        target: str
            Target label
        """
        audio_path = self.get_audio_path(self.audio_dir, self.data_csv.iloc[idx, 0])
    ...
    def __resample(self, wavelet, sample_rate, idx):
        ...
        if sample_rate != self.target_sample_rate:
            try:
                transformation = torchaudio.transforms.Resample(sample_rate, self.target_sample_rate).to(self.device)
    ...
    def __pad_if(self, wavelet):
        ...
        """
        if wavelet.shape[1] < self.num_samples:
            wavelet = nn.functional.pad(wavelet, (0, self.num_samples-wavelet.shape[1]))
        return wavelet
...
def process():
    ...
    dataset = MREData(data_csv, CFG['preprocessing']['audio_dir'], CFG['sample_rate'], CFG['sample_rate']*3, CFG['device'], mel_spectrogram, Amp2db)
