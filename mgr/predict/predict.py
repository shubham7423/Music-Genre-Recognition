...
def __resample(wavelet, sample_rate, target_sample_rate):
    ...
    if sample_rate != target_sample_rate:
        transformation = torchaudio.transforms.Resample(sample_rate, target_sample_rate)
...
def __pad_if(wavelet, num_samples):
    if wavelet.shape[1] < num_samples:
        wavelet = nn.functional.pad(wavelet, (0, num_samples-wavelet.shape[1]))
    return wavelet
...
def get_random_samples(audio_path, num_samples):
    ...
    rand_samples_idx = torch.randperm(audio_samples.size(0))[:min(num_samples, audio_samples.size(0))]
...
def predict(model, audio_file_path, labels=['Electronic', 'Experimental', 'Folk', 'Hip-Hop', 'Instrumental', 'International', 'Pop', 'Rock']):
